...
class ModelMap(Histogram):
    def __init__(self, data, parameter):
        image_axis = HealpixAxis(nside = parameter["nside"],
                                     ...
                                     label = "lb")
        energy_axis = Axis(edges = parameter["energy_edges"] * u.keV, label = "Ei", scale = "log")

        axes = Axes([image_axis, energy_axis])

        super().__init__(axes, unit = 1 / u.s / u.cm / u.cm / u.sr) # unit might be specified in the input parameter.
