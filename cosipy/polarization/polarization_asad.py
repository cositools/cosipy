...
class PolarizationASAD():
    ...
    def create_asad_from_response(self, spectrum, polarization_level, polarization_angle, bins=20):
        ...
        if isinstance(self._convention.frame, SpacecraftFrame):
            ...
            psr = self._response.get_point_source_response(exposure_map=dwell_time_map, coord=self._source_vector.transform_to('galactic'), pol_convention=response_convention)
            ...
            psr = self._response.get_point_source_response(coord=self._source_vector, scatt_map=scatt_map, pol_convention=response_convention)
            expectation = psr.get_expectation(spectrum, LinearPolarization(polarization_level * 100., polarization_angle.angle.deg))
