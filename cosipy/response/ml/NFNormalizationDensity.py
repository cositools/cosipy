...
from .NFBase import DensityApproximation, CompileMode, NFBase, init_density_worker, update_density_worker_settings, DensityModel
from .NFNormalizationDensityModels import Unpolarized2DDensityCMLPDGaussianCARQSFlow, Unpolarized1EMDDensityCMLPDGaussianCARQSFlow


class NormalizationDensityApproximation(DensityApproximation):

    def _setup_model(self):
        version_map: Dict[int, DensityModel] = {
            1: Unpolarized2DDensityCMLPDGaussianCARQSFlow,
            2: Unpolarized1EMDDensityCMLPDGaussianCARQSFlow
        }
...
def init_normalization_density_worker(device_queue: mp.Queue, progress_queue: mp.Queue, major_version: int,
    ...
    init_density_worker(device_queue, progress_queue, major_version,
                        ...
                        density_compile_mode, NormalizationDensityApproximation)

class NFNormalizationDensity(NFBase):
    def __init__(self, path_to_model: Union[str, Path], density_batch_size: int = 100_000,
        ...
        super().__init__(path_to_model, update_density_worker_settings, init_normalization_density_worker, density_batch_size, devices, density_compile_mode, ['is_polarized', 'norm_dimensions'], show_progress)
