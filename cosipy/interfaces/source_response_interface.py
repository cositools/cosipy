...
class ThreeMLModelFoldingInterface(Protocol):
    def set_model(self, model: Model):
        """
        The model is passed as a reference and its parameters can
...
@runtime_checkable
class UnbinnedThreeMLModelFoldingInterface(ThreeMLModelFoldingInterface,
                                           ExpectationDensityInterface,
                                           Protocol):
    """
...
@runtime_checkable
class BinnedThreeMLModelFoldingInterface(ThreeMLModelFoldingInterface,
                                         BinnedExpectationInterface,
                                         Protocol):
    """
    No new methods. Just the inherited ones.
...
class ThreeMLSourceResponseInterface(Protocol):
    ...
    def copy(self) -> "ThreeMLSourceResponseInterface":
        ...
        sources.  It is expected to return a safe copy of itself such
        that when a new source is set, the expectation calculation are
        independent.
...
@runtime_checkable
class UnbinnedThreeMLSourceResponseInterface(ThreeMLSourceResponseInterface,
                                             ExpectationDensityInterface,
                                             Protocol):
    """
    No new methods. Just the inherited ones.
...
class CachedUnbinnedThreeMLSourceResponseInterface(UnbinnedThreeMLSourceResponseInterface, Protocol):
    """
    Guaranteeing that the source response can be cached to and loaded
    from a file.
    ...
    def cache_to_file(self, filename: Union[str, Path]):
        ...
        The implementation has to make sure that the source is handled
        correctly.
    ...
    def init_cache(self):
        """
        Initialize the response cache that can be saved to file.  This way
        there is no need to call expected_counts() or
        expectation_density() to initialize the cache.  Make sure that
        repeated calls don't lead to unnecessary recomputations.
