...
for each in test_list:
    ...
    if np.amax(diff) > 1e-12:
        ...
        print()
        print("WARNING: Definition does not match MEGAlib: %s" %each["name"])
        print("Fraction with diff > 1e-12: " + str(bad_frac))
        print("Max difference: " + str(np.amax(np.absolute(diff))) + " " + str(each["units"]))
        print()
    else:
