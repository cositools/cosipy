...
def test_construct_with_equatorial_position():
        src = CosipyPointSource("test_source", ra=125.6, dec=-75.3, spectral_shape=Powerlaw())
...
def test_construct_with_galactic_position():
        src = CosipyPointSource("test_source", l=15.67, b=80.75, spectral_shape=Powerlaw())
...
def test_construct_with_sky_position_object():
        ...
        src = CosipyPointSource("test_source", sky_position=sky_pos, spectral_shape=Powerlaw())
...
def test_no_position_raises():
        with pytest.raises(AssertionError):
            CosipyPointSource("test_source", spectral_shape=Powerlaw())
...
def test_partial_equatorial_position_raises():
        ...
        with pytest.raises(AssertionError):
            CosipyPointSource("test_source", ra=125.6, spectral_shape=Powerlaw())
...
def test_invalid_ra_dec_type_raises():
        with pytest.raises(AssertionError):
            CosipyPointSource("test_source", ra="not_a_number", dec=-75.3,
                         spectral_shape=Powerlaw())
...
def test_multiple_components():
        ...
        src = CosipyPointSource("test_source", ra=125.6, dec=-75.3, components=[c1, c2])
        assert set(src.components.keys()) == {"component1", "component2"}
        assert len(src.components) == 2
 
def test_no_spectrum_raises():
        with pytest.raises(AssertionError):
            CosipyPointSource("test_source", ra=125.6, dec=-75.3)
...
def test_call_matches_direct_component_evaluation():
        pl = Powerlaw()
        src = CosipyPointSource("test_source", ra=125.6, dec=-75.3, spectral_shape=pl)
...
def _make_source():
        return CosipyPointSource("test_source", ra=125.6, dec=-75.3, spectral_shape=Powerlaw())
